...
<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kabidlatih&opt=berkas_tolak';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kabidlatih/berkas_tolak/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '222' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg = '222' and isDisposisi='0'");
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kabidlatih/berkas_tolak/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kabidlatih/berkas_tolak/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kabidlatih/berkas_tolak/terima2.php');
		else
