...
function hapus(id){
    jConfirm("Anda yakin akan menghapus berita ini?", "PERHATIAN", function(r){
        if(r)
            document.location.href = "php/cp_hapus_berita_dan_informasi.php?id=" + id;
    });
}
function edit(id){
    document.location.href = "?mod=cp_manage_berita_dan_informasi&id=" + id;
}
</script>
<form name="frm" action="?mod=cp_berita_dan_informasi" method="post">
...
<div class="panelcontainer" style="width: 100%;">
    ...
    <div class="bodypanel">
        <input type="button" value="Tambah Berita dan Informasi" onclick="document.location.href='?mod=cp_manage_berita_dan_informasi';" />
        <div class="kelang"></div>
        ...
        ?>
            <tr>
                ...
                <td>
                    <img src='image/Edit-32.png' width='18px' class='linkimage' title='Edit berita' onclick='edit(<?php echo($ds["id"]); ?>);' />
                </td>
                <td>
                    <img src='image/Delete-32.png' width='18px' class='linkimage' title='Hapus berita' onclick='hapus(<?php echo($ds["id"]); ?>);' />
                </td>
            </tr>
        <?php
            }
