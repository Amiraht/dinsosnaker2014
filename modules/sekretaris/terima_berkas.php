...
function tampil(c,uid)
{
		...
		if(t.innerHTML != ''){
			...
			t.innerHTML='<iframe src="./modules/sekretaris/terima_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=sekretaris&opt=terima_berkas';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>MENERIMA BERKAS DAN DISPOSISI</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
                  ...
                  <img src="./image/panah.gif"  /><a href="" id="linkutama">MENERIMA BERKAS DAN DISPOSISI</a> 
                 </td>
...
<tr>
	<td>
		...
		<fieldset>
		<legend>MENERIMA BERKAS DAN DISPOSISI</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '77' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg = '77' and isDisposisi='0'");
...
<?php
		...
		if($_GET["opts"] == 'hubinsaker')
			include('./modules/sekretaris/terima_berkas/ToHubinsaker.php');
		else if($_GET["opts"] == 'binapentaker')
			include('./modules/sekretaris/terima_berkas/ToBinapentaker.php');
		else
