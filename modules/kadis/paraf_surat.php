...
<script type="text/javascript">
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kadis/paraf_surat/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1300px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kadis&opt=paraf_surat';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>TANDA TANGAN SURAT PEMANGGILAN DAN SK PENUGASAN</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
                  ...
                  <img src="./image/panah.gif"  /><a href="" id="linkutama">TANDA TANGAN SURAT PEMANGGILAN DAN SK PENUGASAN</a> 
                 </td>
...
<tr>
    ...
    <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>TANDA TANGAN SURAT PEMANGGILAN DAN SK PENUGASAN</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('33','34','36') and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg in('33','34','36') and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('33','34','36') and isDisposisi='0' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg in('33','34','36') and isDisposisi='0'
											LIMIT ".$posisi.",".$batas."");
						}
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kadis/paraf_surat/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kadis/paraf_surat/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kadis/paraf_surat/terima2.php');
		else
