...
<script type="text/javascript">
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kasiwasnaker/cek_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kasiwasnaker&opt=cek_berkas';
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '121' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '121' and isDisposisi='0'");
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kasiwasnaker/cek_berkas/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kasiwasnaker/cek_berkas/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kasiwasnaker/cek_berkas/terima2.php');
		else
