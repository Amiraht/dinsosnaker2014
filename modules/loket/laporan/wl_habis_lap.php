...
<script type="text/javascript">
	function gocetak(a){
		var x ='./index.php?mod=loket&opt=wl_habis_lap';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=loket&opt=wl_habis_lap';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>LAPORAN SK WAJIB LAPOR YANG BELUM MELAPOR</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <a href='index.php?mod=loket&opt=laporan'> <span></span> LAPORAN-LAPORAN </a>
                     <span> </span> <img src="./image/panah.gif" /> <span> </span><a href='#'>LAPORAN SK WAJIB LAPOR YANG BELUM MELAPOR</a>
                   </td>
...
<tr>
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$d = mysql_query("select count(*) as jumlah from vw_sk_wl where sampai <'$tgl_sekarang' and ".$up."");
	}
	else
	{
		$d = mysql_query("select count(*) as jumlah from vw_sk_wl where sampai <'$tgl_sekarang' ");
	}	
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$query = mysql_query("select no_resi,tgl_masuk,nama,alamat,no_sk,berlaku,sampai from vw_sk_wl where sampai <'$tgl_sekarang' and ".$up." LIMIT ".$posisi.",".$batas."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk,nama,alamat,no_sk,berlaku,sampai from vw_sk_wl where sampai <'$tgl_sekarang' LIMIT ".$posisi.",".$batas."");
	}						
