...
<script type="text/javascript">
	function cetak(u)
	{
		s='./index.php?mod=stafkasiwasnaker&opt=cetak&link='+u;
		window.open(s)
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=stafkasiwasnaker&opt=cetak_ulang';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>PRINT ULANG SK WAJIB LAPOR</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
                  ...
                  <img src="./image/panah.gif"  /><a href="" id="linkutama"> PRINT ULANG SK WAJIB LAPOR</a> 
                 </td>
...
<tr>
    ...
    <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>PRINT ULANG SK WAJIB LAPOR</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_pengurusan='01'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_pengurusan='01'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_pengurusan='01' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/stafkasiwasnaker/cetak_ulang/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/stafkasiwasnaker/cetak_ulang/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/stafkasiwasnaker/cetak_ulang/terima2.php');
		else
