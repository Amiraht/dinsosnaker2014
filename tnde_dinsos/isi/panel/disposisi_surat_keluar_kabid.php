<?php
    ...
    if($_SESSION["id_level"] == 2){
        $judul = "DAFTAR SURAT KELUAR DARI KABID / KABAG YANG AKAN DIPERIKSA";
        $tombol_ok = "Disposisi Ke Kaban";
        $tombol_tolak = "Perlu Perbaikan. Kembalikan Ke Bidang";
    }else{
        $judul = "DAFTAR SURAT KELUAR DARI KASUBBID / KASUBBAG YANG AKAN DIPERIKSA";
...
function disposisi(id, stt){
    jConfirm("Anda yakin akan mendisposisikan data surat masuk ini?", "PERHATIAN", function(r){
        if(r){
            ...
            document.location.href = "php/disposisi_surat_keluar_kabid.php?id=" + id + "&tujuan_disposisi=" + tujuan_disposisi + "&catatan=" + catatan + "&status=" + stt;
        }
    });
}
</script>
