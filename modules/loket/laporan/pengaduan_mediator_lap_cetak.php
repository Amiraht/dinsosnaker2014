...
if ($_GET["mode"]=='1')
{
	...
	HeaderingExcel('Laporan_pengaduan_mediator.xls');
	...
	// tabel title=================================================================================
	$worksheet1->write_string(2,1,"LAPORAN PENGADUAN yang ditangani mediator",$fBesar);
	...
	$worksheet1->write_string(4,9,"Mediator",$fList_title);
	...
	{
		$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon,mediator from vw_info_berkas where id_proses_skrg='61' and isDisposisi='0' and ".$up."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon,mediator from vw_info_berkas where id_proses_skrg='61' and isDisposisi='0' and id_pengurusan='03'");
	}			
	...
	while($m1 = mysql_fetch_row($query))
	{
		...
		$q1="select nama from user where id_user='".$m1[7]."'";
		...
		$worksheet1->write_string($i+4,9,$nama,$fList);
		$i++;
	}
	$workbook->close();
}
