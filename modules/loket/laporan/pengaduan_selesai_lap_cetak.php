...
if ($_GET["mode"]=='1')
{
	...
	HeaderingExcel('Laporan_pengaduan_selesai.xls');
	...
	// tabel title=================================================================================
	$worksheet1->write_string(2,1,"LAPORAN PENGADUAN YANG SUDAH SELESAI",$fBesar);
	...
	$worksheet1->write_string(4,9,"Hasil",$fList_title);
	...
	{
		$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where id_proses_skrg='81' and isDisposisi='0' and ".$up."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where id_proses_skrg='81' and isDisposisi='0' and id_pengurusan='03'");
	}			
	...
	while($m1 = mysql_fetch_row($query))
	{
		...
		$med=mysql_query("select hasil_mediasi from tbl_berkas_pengaduan where no_resi='".$m1[0]."'");
		...
		$worksheet1->write_string($i+4,9,$hsl1[0],$fList);
		$i++;
	}
	$workbook->close();
}
