...
function toggle(a) {
	...
	function goto(x){
		var a = "./index.php?mod=loket&opt=penyerahan_berkas&opts=done";
...
WHERE c.no_resi=a.no_resi AND a.isSelesai='1' AND a.isValid='1' AND a.arsip_akhir='0'");						
?>
<table border='1' class="tblisi" style='width:100%; margin-bottom:10px;'>
				...
				<?php 
					...
					echo "<tr >
						...
						<input type="button" onClick="CekAll('<?php echo $set[0]; ?>')" value="CENTANG SEMUA"> <input type="button" id="btn<?php echo $set[0]; ?>" value="PROSES" onClick="goto('<?php echo $set[0]; ?>')" disabled>
						<?php
						echo"</div>
								</div>
							</td>
...
<?php 
if($_GET["opts"]==''){
} else if($_GET["opts"] == 'done') 
			include('./modules/loket/berkas_akhir/selesai.php');//sudah
		else
