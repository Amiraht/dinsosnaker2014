...
function tampil(c,uid)
{
		...
		if(t.innerHTML != ''){
			...
			t.innerHTML='<iframe src="./modules/kadis/terima_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1300px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kadis&opt=terima_berkas';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>MENERIMA BERKAS DAN MELAKUKAN DISPOSISI</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <img src="./image/panah.gif"  /><a href="./index.php?mod=kadis&opt=main" id="linkutama"> BERANDA LOGIN KADIS</a> <span>
                  <img src="./image/panah.gif"  /><a href="" id="linkutama">MENERIMA BERKAS DAN MELAKUKAN DISPOSISI</a> 
                 </td>
...
<tr>
    ...
    <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>MENERIMA BERKAS DAN MELAKUKAN DISPOSISI</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('31','38') and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg in('31','38') and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$str = "SELECT 
                                ...
                                WHERE 
									...
									a.id_proses_skrg in('31','34','38')  AND 
									a.isDisposisi = '0' 
								LIMIT 
									 ".$posisi." , ".$batas." 				
						";
			...
			?>
			<table class='tblisi' cellspacing="0" cellpadding="5" style='width:100%;'>
					<tr align="center">
...
<?php
		...
		if($_GET["opts"] == 'hubinsaker')
			include('./modules/kadis/terima_berkas/ToHubinsaker.php');
		else if($_GET["opts"] == 'binapentaker')
			include('./modules/kadis/terima_berkas/ToBinapentaker.php');
		else
