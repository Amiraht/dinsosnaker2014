...
<div class="kelang"></div>
<div class="panelcontainer" style="width: 100%;">
    <h3>REKAP SURAT YANG DITERIMA</h3>
    <div class="bodypanel">
    ...
    <?php
        $whr = "";
        $sql = "";
        if($_GET["id_level"]==1 || $_GET["id_level"]==2 || $_GET["id_level"]==18){
            if($_POST["no_surat"] <> "")
            ...
            $ds_urutan = mysql_fetch_array(mysql_query("SELECT * FROM myapp_reftable_levelpengguna WHERE id='" . $_GET["id_level"] . "'"));
            $urutan = $ds_urutan["urutan"];
            
            $tmbh = " (c.id_level_tujuan = '" . $_GET["id_level"] . "' ";
            if($urutan == 2){
                $res_bawahan = mysql_query("SELECT * FROM myapp_reftable_levelpengguna WHERE atasan='" . $_GET["id_level"] . "'");
                ...
                $res_bawahan = mysql_query("SELECT * FROM myapp_reftable_levelpengguna WHERE atasan='" . $_GET["atasan"] . "' AND urutan=4");
                while($ds_bawahan = mysql_fetch_array($res_bawahan)){
                    $tmbh .= " OR ";
                    $tmbh .= " (c.id_level_asal = '" . $_GET["id_level"] . "' AND c.id_level_tujuan = '" . $ds_bawahan["id"] . "') ";
                    $tmbh .= " OR ";
                    $tmbh .= " (c.id_level_asal = '" . $ds_bawahan["id"] . "' AND c.id_level_tujuan = '" . $_GET["id_level"] . "') ";
                }
                $res_bawahan_langsung = mysql_query("SELECT * FROM myapp_maintable_pengguna WHERE level_kasubbid='" . $_GET["id_level"] . "'");
            ...
            $sql = "SELECT 
                    ...
                    WHERE
                    	1 AND c.id_pengguna_tujuan = '" . $_GET["id_pengguna"] . "'
                    GROUP BY
                        a.id
                    ORDER BY 
                    	id DESC";
    ...
    <div class="kelang"></div>
    <input type="button" value="Kembali" onclick="document.location.href='?mod=laporan_rekap_total';" />
    </div>
</div>
<!-- DIALOG -->
    ...
    <div class="panelcontainer" style="width: 100%;">
        <div class="bodypanel">
            <table border="0px" cellspacing='0' cellpadding='0' width='100%'>
                ...
                <?php
                    $res_ldb = mysql_query("SELECT * FROM myapp_reftable_levelpengguna WHERE atasan='" . $_GET["id_level"] . "' AND id <> 2");
