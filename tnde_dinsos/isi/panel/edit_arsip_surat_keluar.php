...
<form name="frm" action="?mod=edit_arsip_surat_keluar" method="post">
...
<div class="kelang"></div>
<div class="panelcontainer" style="width: 100%;">
    <h3>CARI SURAT KELUAR YANG AKAN DIEDIT DATA PENGARSIPKAN</h3>
    <div class="bodypanel">
    ...
    <?php
        ...
        $res = mysql_query("SELECT 
                            ...
                            FROM 
                            	...
                            	LEFT JOIN myapp_archivetable_suratkeluar c ON a.id = c.id_surat_keluar
                            WHERE
                            	1 AND c.id_surat_keluar IS NOT NULL " . $whr . "
                            ORDER BY 
                            	id ASC");
        $ctr = 0;
        while($ds = mysql_fetch_array($res)){
            ...
            echo("<tr>");
                ...
                echo("<td align='center'>");
                    echo("<img src='image/information_32.png' width='18px' class='linkimage' title='Detail Surat Keluar' onclick='lihat_detail_sk(" . $ds["id"] . ", 0);'>");
                echo("</td>");
                echo("<td align='center'>");
                    echo("<img src='image/icon-disposisi.png' width='18px' class='linkimage' title='Daftar catatan disposisi' onclick='lihat_cadis_sk(" . $ds["id"] . ", 0);'>");
                echo("</td>");
                echo("<td align='center'>");
                    echo("<img src='image/Attachment-32.png' width='18px' class='linkimage' title='File yang dilampirkan' onclick='lihat_file_sk(" . $ds["id"] . ", 0);'>");
                echo("</td>");
                echo("<td align='center'>");
                    echo("<img src='image/Button Next_32.png' width='18px' class='linkimage' title='Arsipkan Surat Keluar' onclick='document.location.href=\"?mod=pengarsipan_surat_keluar&id=" . $ds["id"] . "\"'>");
                echo("</td>");
            echo("</tr>");
        }
    ?>
