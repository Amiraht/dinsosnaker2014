...
if($no_resi<>'' and $judul<>'')
{
	if($mode == 1)
	{
		...
		else
		{
			...
			{
				$tambah_type = mysql_query("insert into tbl_hasil_tugas (judul, no_resi, file, tanggal) values('$judul', '$no_resi', '$nama', '".date("Y-m-d")."')");
				if(!tambah_type)
				{
				?>
				<script type="text/javascript">
	...
	else
	{
		...
		if(!empty($_GET["link"]))
		{
			$q3 = mysql_query("select * from tbl_hasil_tugas where no_resi='".$_GET["link"]."'");
			...
			if(!empty($_FILES["file"]["name"]))
			{
				...
				if(move_uploaded_file($filesementara, $path.$nama))
					$edit = mysql_query("update tbl_hasil_tugas set judul='".$judul."', file='".$nama."' where no_resi='".$_GET["link"]."'");	
			}
			else
			{
				$edit = mysql_query("update tbl_hasil_tugas set judul='".$judul."' where no_resi='".$_GET["link"]."'");
			}		
