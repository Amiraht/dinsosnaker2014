...
<script type="text/javascript">
	function gocetak(a){
		var x ='./index.php?mod=loket&opt=dpkk_lap';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=loket&opt=dpkk_lap';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>LAPORAN JUMLAH SETORAN DPKK</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <a href='index.php?mod=loket&opt=laporan'> <span></span> LAPORAN-LAPORAN </a>
                     <span> </span> <img src="./image/panah.gif" /> <span> </span><a href='#'>LAPORAN JUMLAH SETORAN DPKK</a>
                   </td>
...
<tr>
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$d = mysql_query("select count(*) as jumlah,sum(jlh_setoran_dpkk) as setoran from vw_imta_baru where  nama_ta is not null  and ".$up."");
	}
	else
	{
		$d = mysql_query("select count(*) as jumlah,sum(jlh_setoran_dpkk) as setoran from vw_imta_baru where nama_ta is not null");
	}	
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$query = mysql_query("select no_resi,tgl_masuk_berkas,nama_ta,alamat_ta,kewarganegaraan,jlh_setoran_dpkk from vw_imta_baru where nama_ta is not null and ".$up." LIMIT ".$posisi.",".$batas."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk_berkas,nama_ta,alamat_ta,kewarganegaraan,jlh_setoran_dpkk from vw_imta_baru where nama_ta is not null LIMIT ".$posisi.",".$batas."");
	}						

	if($_GET["rekap"]==1)
	{
		echo 'HASIL REKAPITULASI = '.$jmldata ;
		echo '<br><br>TOTAL SETORAN = '.number_format($jmlsetoran) ;
	}
	else
	{							
	...
	?>
    <a href="./modules/loket/laporan/dpkk_lap_cetak.php?mode=1&tgl1=<?php echo $_GET["tgl1"]; ?>&tgl2=<?php echo $_GET["tgl2"]; ?>&cari=<?php echo $_GET["cari"]; ?>&no_resi=<?php echo $_GET["no_resi"]; ?>&pengurusan=<?php echo $_GET["pengurusan"]; ?>" target="_blank" style="border:none"><img src="./image/printer.png" width="35" height="35" border="0" title="Export to Excel"/></a><br /><br />       
