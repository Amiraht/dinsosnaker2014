...
<script type="text/javascript">
	...
	function nomor(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/penomoran/sk_tugas/index.php?link='+c+'&id_user='+uid+'" width="100%" height="400px" frameborder="0" scrolling="no" id="iframe_detail"></iframe>';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kasubag_umum/penomoran_surat/tampil.php?link='+c+'&id_user='+uid+'" width="100%" height="450px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kasubag_umum&opt=penomoran_surat';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>PENOMORAN SURAT-SURAT</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
                  ...
                  <img src="./image/panah.gif"  /><a href="" id="linkutama"> PENOMORAN SURAT-SURAT </a> 
                 </td>
...
<tr>
	<td>
		...
		<fieldset>
		<legend>PENOMORAN SURAT-SURAT</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('22','24','26') and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg in('22','24','26') and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$str = "SELECT 
                                ...
                                WHERE 
									...
									a.id_proses_skrg IN ('22','24','26') AND 
									a.isDisposisi = '0' 
								LIMIT 
									 ".$posisi." , ".$batas." 				
						";
			...
			<table border='1' class='tblisi' cellspacing="0" style='width:100%; margin-bottom:10px;'>
					<tr align="center">
						...
						<th colspan="2" align="center">AKTIFITAS</th>
					</tr>
				<?php 
					...
					echo "<tr >
						...
						while($set = mysql_fetch_array($query))
						{
							...
							$onc1 = "nomor('".$set['no_resi']."','".$_SESSION["id_user"]."')";
							$no=0;
							if($halaman>1)
							{
								$no=($halaman-1)*20;
					...
					echo "
					<tr id='div_if'>
						<td colspan='11'> <div id='div_cek_".$set['no_resi']."'></div>
					</tr>
					";
						}
					?>
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kasubag_umum/penomoran_surat/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kasubag_umum/penomoran_surat/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kasubag_umum/penomoran_surat/terima2.php');
		else
