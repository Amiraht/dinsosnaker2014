...
function proses_perilaku(stt){
    frm.stt.value = stt;
    if(stt == 2){
        var keberatan = frm.keberatan.value;
        if(keberatan == "")
            jAlert("Maaf, jika keberatan harap diisikan alasan \"keberatan Dari PNS yang dinilai\"", "PERHATIAN");
        else
            frm.submit();
    }else{
        frm.submit();
        	...
        	<div id='header' style='background:url(./image/coba/header2.png) no-repeat;'>
              <table border="0" id='atasan'>
                        <tr>
                                ...
                                <h1><a style="color:#AA9F00;" href="">LIHAT DATA PERILAKU</a></h1>
                                </td>
                        </tr>
                        <tr>
                            <td style="text-align:left; padding-left:10px; border-left:0px solid black;"><a href='./'> <span></span> BERANDA UTAMA </a>
                                 <span> </span> <img src="./image/panah.gif" /> <span>LIHAT DATA PERILAKU</span></td>
  ...
  <tr>
    ...
    <div id='content' style='padding-top:10px;'>	<br>
		...
		<div class="panelcontainer panelform" style="width: 100%;">
			<h3 style="text-align: left;">DATA PERIODE <?php echo(strtoupper(detail_pegawai($_SESSION["id_pegawai"], "nama_pegawai"))); ?> (NIP : <?php echo(strtoupper(detail_pegawai($_SESSION["id_pegawai"], "nip"))); ?>)</h3>
		...
		<?php
			$ds_skp = mysql_fetch_array(mysql_query("SELECT * FROM tbl_skp_perilaku WHERE id_skp_perilaku='" . $id_skp . "'"));
		?>
		<form>
		<input type="hidden" name="id_skp" value="<?php echo($_GET["id_skp"]); ?>" />
		<div class="panelcontainer panelform" style="width: 100%;">
			<h3 style="text-align: left;">ISI DATA PENILAIAN PERILAKU PEGAWAI <?php echo(strtoupper(detail_pegawai($_SESSION["id_pegawai"], "nama_pegawai"))); ?> (NIP : <?php echo(strtoupper(detail_pegawai($_SESSION["id_pegawai"], "nip"))); ?>)</h3>
			<div class="bodypanel bodyfilter" id="bodyfilter">
				<input type="button" value="Kembali" onclick="document.location.href='?mod=perilaku_lihat_pilih_periode';" />
				<div class="kelang"></div>
		...
		<div class="kelang"></div>
		<form name="frm" id="frm" action="php/skp/perilaku_lihat.php" method="post">
		<input type="hidden" name="id_skp" value="<?php echo($_GET["id_skp"]); ?>" />
		<input type="hidden" name="stt" value="" />
		<div class="panelcontainer panelform" style="width: 100%;">
			<h3 style="text-align: left;">ISI DATA KEBERATAN, TANGGAPAN, DAN KEPUTUSAN  PERILAKU PEGAWAI <?php echo(strtoupper(detail_pegawai($_SESSION["id_pegawai"], "nama_pegawai"))); ?> (NIP : <?php echo(strtoupper(detail_pegawai($_SESSION["id_pegawai"], "nip"))); ?>)</h3>
			<div class="bodypanel bodyfilter" id="bodyfilter">
				<table border="0px" cellspacing='0' cellpadding='0' width='100%'>
					<tr>
						<td>
							<label>Keberatan Dari PNS Yang Dinilai :</label>
							<textarea name="keberatan" placeholder="Jika Anda Keberatan dengan Hasil Penilaian Perilaku, Harap isi Input Keberatan Ini."><?php echo($ds_skp["keberatan"]); ?></textarea>
						</td>
					</tr>
				</table>
				<?php
				...
				<table border="0px" cellspacing='0' cellpadding='0' width='100%'>
					<tr>
						<td style="text-align: left;">
							<input name="terima" type="button" value="TERIMA" style="width: 150px; height: 40px;" onclick="proses_perilaku(1);" />
							<input name="tolak" type="button" value="KEBERATAN" style="width: 150px; height: 40px;" onclick="proses_perilaku(2);" />
						</td>
					</tr>
				</table>
				<?php
					}else if($ds_skp["status_supervisi"] == 2){
						echo("<div style='font-family: verdana; font-weight: light; color: #458B00; font-size: 9pt; padding: 3px 10px; background-color: #EEEEEE; margin-bottom: 10px;'>Data keberatan anda sedang di proses oleh Pejabat Penilai</div>");
					}else if($ds_skp["status_supervisi"] == 3){
						echo("<div style='font-family: verdana; font-weight: light; color: #458B00; font-size: 9pt; padding: 3px 10px; background-color: #EEEEEE; margin-bottom: 10px;'>Data keberatan anda sedang di proses oleh Atasan Pejabat Penilai</div>");
					}else if($ds_skp["status_supervisi"] == 4){
						echo("<div style='font-family: verdana; font-weight: light; color: #458B00; font-size: 9pt; padding: 3px 10px; background-color: #EEEEEE; margin-bottom: 10px;'>Data perilaku telah disimpan</div>");
					}
				?>
				<table border="0px" cellspacing='0' cellpadding='0' width='100%'>
					<tr>
