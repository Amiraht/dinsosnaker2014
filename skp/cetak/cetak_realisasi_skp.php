...
<?php
			...
			$id = mysql_real_escape_string(strip_tags($_GET['id_skp']));
            ...
            $ds_header = mysql_fetch_array($sql);
 ...
 */
    ...
    $html .= "<table width='100%' cellspacing='0' cellpadding='5' border='0px' style='font-family: sans-serif; font-size: 9.5pt;'>";
        ...
        $res_data = mysql_query("SELECT
                                    CASE
                            			WHEN a.id_uraian_skp IS NULL THEN 0
                            			ELSE 1
                            		END AS ada,
                                    fx.kegiatan AS fx_kegiatan, fx.kuantitas AS fx_kuantitas,
                                    a.*, c.satuan_waktu, 
                                	CASE
                                		WHEN SUM(b.kuantitas) IS NULL THEN 0
                                		ELSE SUM(b.kuantitas)
                                	END AS rel_kuantitas,
                                	(SELECT AVG(kualitas) FROM tbl_uraian_realisasi_skp WHERE id_skp='" . $id . "' AND id_uraian_skp=b.id_uraian_skp AND kualitas > 0) AS rel_kualitas,
                                	CASE 
                                ...
                                FROM
                                    ...
                                    RIGHT JOIN tbl_uraian_skp_fix fx ON a.id_uraian_skp = fx.id_uraian_skp
                                WHERE
                                	fx.id_skp = '" . $id . "'
                                GROUP BY
                                	fx.id_uraian_skp
                                ORDER BY
                              		fx.id_uraian_skp ASC") or die(mysql_error());
        $no = 0;
        $total_seluruh = 0;
        while($ds_data = mysql_fetch_array($res_data)){
            ...
            $html .= "<tr>";
                ...
                $html .= "<td style='text-align: left; border: solid 1px #000000; " . $overline . "' >" . $ds_data["fx_kegiatan"] . "</td>";
                
                if($ds_data["ada"] == 1){ // START OF ADA *********************************************************
                    $html .= "<td style='text-align: center; border: solid 1px #000000;'>" . $tulis_target_ak . "</td>";
                    if($ds_data["fx_kuantitas"] == $ds_data["kuantitas"]){
                        $html .= "<td style='text-align: center; border: solid 1px #000000;'>" . $ds_data["kuantitas"] . " " . $ds_data["satuan_kuantitas"] . "</td>";
                    }else{
                        $html .= "<td style='text-align: center; border: solid 1px #000000;'><span style='text-decoration: line-through;'>" . $ds_data["fx_kuantitas"] . "</span><br/>" . $ds_data["kuantitas"] . " " . $ds_data["satuan_kuantitas"] . "</td>";
                    }
        ...
        $nilai_skp = $total_seluruh / $no;
            ...
            $html .= "<tr>";
                ...
                $html .= "<td style='text-align: center; border: solid 1px #000000;'>" . penilaian_tugas_tambahan($no_tugas_tambahan) . "</td>";
            $html .= "</tr>";
            ...
            while($ds_kreatifitas = mysql_fetch_array($res_kreatifitas)){
                ...
                $html .= "</tr>";
                $total_kreatifitas += $ds_kreatifitas["nilai"];
            }
            $html .= "<tr>";
                $html .= "<td style='text-align: center; border: solid 1px #000000;'>&nbsp;</td>";
                $html .= "<td style='text-align: left; border: solid 1px #000000;' colspan='12'><b>TOTAL KREATIFITAS</b></td>";
                $html .= "<td style='text-align: center; border: solid 1px #000000;'>" . $total_kreatifitas . "</td>";
            $html .= "</tr>";
    ...
    $html .= "<table width='100%' cellspacing='0' cellpadding='5' border='0px' style='font-family: sans-serif; font-size: 10pt;'>";
        ...
        $html .= "<tr>";
            $html .= "<td>&nbsp;</td>";
            $html .= "<td align='center'>" . $gelar_depan_penilai . $ds_header["nama_pegawai_penilai"] . $gelar_belakang_penilai . "</td>";
        $html .= "</tr>";
        $html .= "<tr>";
            $html .= "<td>&nbsp;</td>";
