...
<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=staf&opt=cek_berkas';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/staf/cek_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1200px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
...
<tr>
	<td>
     <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
        ...
        <fieldset>
		<legend>PERIKSA BERKAS DAN BUAT SURAT PEMANGGILAN</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '61' and isDisposisi='0' and mediator='".$userid."'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '61' and isDisposisi='0' and mediator='$userid'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '61' and isDisposisi='0' and mediator='$userid' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg = '61' and isDisposisi='0' and mediator='$userid'
											LIMIT ".$posisi.",".$batas."");
						}
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/staf/cek_berkas/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/staf/cek_berkas/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/staf/cek_berkas/terima2.php');
		else if($_GET["opts"] == 'laporan')
			include('./modules/staf/cek_berkas/edit_pengaduan.php');
		else
