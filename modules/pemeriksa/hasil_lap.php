...
<script type="text/javascript">
	...
	function simpan(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/simpan/lap/index.php?link='+c+'&id_user='+uid+'" width="100%" height="400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/pemeriksa/hasil_lap/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=pemeriksa&opt=hasil_lap';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>LAPORAN HASIL VERIFIKASI</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
                  ...
                  <img src="./image/panah.gif"  /><a href="" id="linkutama"> LAPORAN HASIL VERIFIKASI</a>
                 </td>
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '92' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '92' and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '92' and isDisposisi='0' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg = '92' and isDisposisi='0'
											LIMIT ".$posisi.",".$batas."");
						}
				...
				<?php 
					...
					echo "<tr>
						...
						while($set = mysql_fetch_array($query))
						{
							...
							$onc1 = "simpan('".$set[0]."','".$_SESSION["id_user"]."')";
							$no=0;
							if($halaman>1)
							{
								$no=($halaman-1)*20;
							...
							echo "<td> $set[6]</td>"; 
							echo "
								<td> <input type='button' value='LAPORAN' onClick=$onc1></td>
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/pemeriksa/hasil_lap/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/pemeriksa/hasil_lap/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/pemeriksa/hasil_lap/terima2.php');
		else
