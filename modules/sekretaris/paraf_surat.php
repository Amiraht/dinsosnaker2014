...
<script type="text/javascript">
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/sekretaris/paraf_surat/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1000px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=sekretaris&opt=paraf_surat';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>PARAF SURAT PEMANGGILAN DAN PENUGASAN</a></b></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <img src="./image/panah.gif"  /><a href="./index.php?mod=sekretaris&opt=main" id="linkutama"> BERANDA LOGIN SEKRETARIS</a>
                  <img src="./image/panah.gif"  /><a href="" id="linkutama">PARAF SURAT PEMANGGILAN DAN PENUGASAN</a> 
                 </td>
...
<td style="float:right;">
					<img width="90" height="29" 
						onclick="document.location.href='./index.php?mod=sekretaris&opt=main'" 
...
<tr><td>&nbsp;</td></tr>
<tr>
	<td> <div id='content' style='margin-left:1%;margin-bottom:20px;width:99%;'>
        <fieldset>
            ...
            <tr><td></td><td><input type='button' value='CARI' onclick="Kirim_Cari();"></td></tr></table></br>
            </form>
        </fieldset>
		<fieldset>
		<legend>PARAF SURAT PEMANGGILAN DAN PENUGASAN</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('72','75','76','78') and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg in('72','75','76','78') and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('72','75','76','78') and isDisposisi='0' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg in('72','75','76','78') and isDisposisi='0'
											LIMIT ".$posisi.",".$batas."");
						}
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/sekretaris/paraf_surat/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/sekretaris/paraf_surat/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/sekretaris/paraf_surat/terima2.php');
		else
