...
if ($_GET["mode"]=='1')
{
	...
	HeaderingExcel('Laporan_imta_habis.xls');
	...
	// tabel title=================================================================================
	$worksheet1->write_string(2,1,"LAPORAN IMTA YANG BELUM PERPANJANG",$fBesar);
	...
	$worksheet1->write_string(4,7,"Jumlah Setoran",$fList_title);
	$worksheet1->write_string(4,8,"Tanggal IMTA",$fList_title);
	$worksheet1->write_string(4,9,"Tanggal Berakhir",$fList_title);
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$query = mysql_query("select no_resi,tgl_masuk_berkas,nama_ta,alamat_ta,kewarganegaraan,jlh_setoran_dpkk,tgl_imta,tgl_berakhir from vw_imta_baru where nama_ta is not null and tgl_berakhir < '$tgl_sekarang' and ".$up." ");

	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk_berkas,nama_ta,alamat_ta,kewarganegaraan,jlh_setoran_dpkk,tgl_imta,tgl_berakhir from vw_imta_baru where nama_ta is not null and tgl_berakhir < '$tgl_sekarang' ");
	...
	while($m1 = mysql_fetch_row($query))
	{
		...
		$worksheet1->write_string($i+4,9,$m1[7],$fList);
		$i++;
	}
	$workbook->close();
}
