...
if($bz[0]=='221')
{
	if($sta=='terima')
	{
		$qry="select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses = '".$bz[0]."' and id_pengguna='23'";
	}
	else
	{
		$qry="select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses = '".$bz[0]."' and id_pengguna='3'";
...
<fieldset style='width:90%; margin-left:40px; padding-left:20px;min-height:40px; margin-bottom:10px;padding-right:20px;'>
    ...
    <br />
    <br /> 
<form action="./modules/kabidlatih/lembar_disposisi/disposisi_validate.php?link=<?php echo $resi; ?>&id_user=<?php echo $userid; ?>" method="POST">
<table>
<tr>
	<td>DISPOSISI
