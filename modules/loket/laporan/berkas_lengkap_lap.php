<script type="text/javascript">
	function gocetak(a){
		var x ='./index.php?mod=loket&opt=berkas_lengkap_lap';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=loket&opt=berkas_lengkap_lap';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>LAPORAN BERKAS YANG HARUS DILENGKAPI</a></b></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <a href='index.php?mod=loket&opt=laporan'> <span></span> LAPORAN-LAPORAN </a>
                     <span> </span> <img src="./image/panah.gif" /> <span> </span><a href='#'>LAPORAN BERKAS YANG HARUS DILENGKAPI</a>
                   </td>
...
<tr>
	...
	if(!empty($_GET["cari"]) || !empty($_GET["no_resi"]) || !empty($_GET["pengurusan"]))
	{
		$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg in('14') and ".$up."");
	}
	else
	{
		$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg in('14')");
	}	
	...
	if(!empty($_GET["cari"]) || !empty($_GET["no_resi"]) || !empty($_GET["pengurusan"]))
	{
		$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where id_proses_skrg 
							  in('14') and ".$up." LIMIT ".$posisi.",".$batas."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where id_proses_skrg 
							  in('14') LIMIT ".$posisi.",".$batas."");
	}						
	...
	?>
    <a href="./modules/loket/laporan/berkas_lengkap_lap_cetak.php?mode=1&tgl1=<?php echo $_GET["tgl1"]; ?>&tgl2=<?php echo $_GET["tgl2"]; ?>&cari=<?php echo $_GET["cari"]; ?>&no_resi=<?php echo $_GET["no_resi"]; ?>&pengurusan=<?php echo $_GET["pengurusan"]; ?>" target="_blank" style="border:none"><img src="./image/printer.png" width="35" height="35" border="0" title="Export to Excel"/></a><br /><br />       
