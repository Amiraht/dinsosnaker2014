<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=arsip&opt=pengrsipan_edit';
		...
		{
			s=s+'&cari='+document.fcari.jenis.value;
		}
...
function edit_data(c)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/arsip/edit/tambah.php?link='+c+'" width="100%" style="height:800px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>LIHAT DAN EDIT ARSIP</b></a></td>
            </tr>
            <tr>
                 ...
                 <img src="./image/panah.gif"  /><a href="" id="linkutama"> LIHAT DAN EDIT ARSIP</a> 
                 </td>
...
<tr>
    ...
    <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
        ...
        <fieldset>
		<legend>LIHAT DAN EDIT ARSIP</legend>
	<?php
			if(!empty($_GET["cari"]) and !empty($_GET["jenis"]))
				$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' and jenis_pengurusan='".$_GET["jenis"]."' AND id_proses_skrg = '81' and isDisposisi='1'");
			elseif(!empty($_GET["cari"]))
				$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '81' and isDisposisi='1'");
			elseif(!empty($_GET["jenis"]))
				$d = mysql_query("select count(*) as jumlah from vw_info_berkas where jenis_pengurusan='".$_GET["jenis"]."' AND id_proses_skrg = '81' and isDisposisi='1'");				
			else
				$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '81' and isDisposisi='1'");
				...
				if(!empty($_GET["cari"]) and !empty($_GET["jenis"]))
					$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' and jenis_pengurusan='".$_GET["jenis"]."' AND id_proses_skrg = '81' and isDisposisi='1' LIMIT ".$posisi.",".$batas."");
				elseif(!empty($_GET["cari"]))
					$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '81' and isDisposisi='1' LIMIT ".$posisi.",".$batas."");
				if(!empty($_GET["jenis"]))
					$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where jenis_pengurusan='".$_GET["jenis"]."' AND id_proses_skrg = '81' and isDisposisi='1' LIMIT ".$posisi.",".$batas."");
				else
					{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg = '81' and isDisposisi='1'
											LIMIT ".$posisi.",".$batas."");
						}
