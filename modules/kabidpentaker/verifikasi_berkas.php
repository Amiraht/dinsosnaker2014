...
<script type="text/javascript">
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kabidpentaker/verifikasi_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1500px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kabidpentaker&opt=verifikasi_berkas';
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '45' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg = '45' and isDisposisi='0'");
...
<?php
		...
		if($_GET["opts"] == 'tolak')
			include('./modules/kabidpentaker/verifikasi_berkas/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kabidpentaker/verifikasi_berkas/terima.php');
		else
