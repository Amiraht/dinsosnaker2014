...
<script type="text/javascript">
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kabidlatih/verifikasi_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1800px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kabidlatih&opt=verifikasi_berkas';
      ...
      <table border="0" id='atasan'>
            ...
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <img src="./image/panah.gif"  /><a href="./index.php?mod=kabidlatih&opt=main" id="linkutama"> BERANDA LOGIN KABID LATIH</a>
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '221' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg = '221' and isDisposisi='0'");
...
<?php
		...
		if($_GET["opts"] == 'tolak')
			include('./modules/kabidlatih/verifikasi_berkas/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kabidlatih/verifikasi_berkas/terima.php');
		else
