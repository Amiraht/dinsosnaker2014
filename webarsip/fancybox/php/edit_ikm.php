<?php
    ...
    if(!isset($_POST["a"]) || !isset($_POST["b"]) || !isset($_POST["c"]) || !isset($_POST["d"]) || !isset($_POST["e"]) || 
        ...
        !isset($_POST["k"]) || !isset($_POST["l"]) || !isset($_POST["m"]) || !isset($_POST["n"])){
            header("location:../?mod=info&pesan=4&redir=edit_ikm");
    }else{
        mysql_query("DELETE FROM tbl_jawaban_ikm WHERE id_anggota = " . $_SESSION["id_pengguna"] . " AND YEAR(tanggal) = YEAR(CURDATE())");
