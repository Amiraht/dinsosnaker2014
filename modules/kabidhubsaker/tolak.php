...
function tampil(c)
{
		...
		if(t.innerHTML != ''){
			...
			t.innerHTML='<iframe src="./modules/kabidhubsaker/berkas_tolak/tampil.php?link='+c+'" width="100%" style="height:1800px" frameborder="0" scrolling="no" id="iframe_detail"></iframe>';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'">
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00; href="#"><b>CEK BERKAS - KABID HUBIN SAKER</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <img src="./image/panah.gif"  /><a href="./index.php?mod=kabidhubsaker&opt=main" id="linkutama"> BERANDA LOGIN KABID HUBIN SAKER</a>
...
<tr>
	...
	<?php
			if(!empty($_POST["cari"]))
				{
					...
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas WHERE id_proses_skrg='42' AND isDisposisi='0'");
				...
				if(isset($_POST["cari"]))
					{
					...
					else
						{
							$query = mysql_query("SELECT
											...
											WHERE		
												a.id_proses_skrg = '42' AND a.isDisposisi = '0'
											LIMIT ".$posisi.",".$batas."");
						}
			...
			?>
			<table border='1' class='tblisi' cellspacing="0" style='width:95%; margin-bottom:10px;'  width="100%">
					<tr>
...
<?php
		...
		if($_GET["opts"] == 'tolak')
			include('./modules/kabidhubsaker/berkas_tolak/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kabidhubsaker/berkas_tolak/terima.php');
		else
