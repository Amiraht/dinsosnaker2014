<?php
	...
	$sql = "SELECT * FROM tbl_sp2 WHERE no_resi='".$ab."'";
	$res = mysql_query($sql);
	$ds = mysql_fetch_array($res);
	
	$no_surat1=$ds["no_surat"];
	$tgl_surat1=$ds["tanggal"];
	$tgl_h1=$ds["tgl_h"];
	
	$sql = "SELECT * FROM tbl_sp3 WHERE no_resi='".$ab."'";
	$res = mysql_query($sql);
	$ds = mysql_fetch_array($res);
	...
	$html = '<table border="0" width="100%" align="center">
			...
			<tr>
				...
				<td width="55%" align="left">: <u>Panggilan III</u></td>
...
<p>Berdasarkan Undang-Undang No. 13 Tahun 2003 dan Undang-Undang No. 2 Tahun 2004, sehubungan dengan surat panggilan II Kepala Dinas Sosial dan Tenaga Kerja Kota Medan No. '.$no_surat1.' tanggal '. tgl_indo($tgl_surat1).' untuk berunding tanggal '.tgl_indo($tgl_h1).' perihal '.$masalah.' dari '.$per.' maka untuk penyelesaian masalah tersebut diminta kehadiran saudara pada : 
			</p>
