...
$konsep_pp = anti_injection($_POST["konsep_pp"]);
$tgl_pp = anti_injection($_POST["tgl_pp"]);
...
if((int)$_GET["mode"] == 1)
{
	if($nama_pemohon == '' or $alamat_pemohon == '' or $no_surat == '' or $id_per == '' or $tgl_surat == '' or $no_jamsostek == '' or $tenaga_lk == '' or $tenaga_pr == '' or $tenaga_lk_cabang == '' or $tenaga_pr_cabang == '' or $upah_bulanan_max == '' or $upah_bulanan_min == '' or $upah_harian_max == '' or $upah_harian_min == '' or $hub_tentu == '' or $hub_tak_tentu == '' or $konsep_pp == '' or $tgl_pp == '')
	{
	?>
		<script type="text/javascript">
			alert('Isi dengan lengkap !');
			document.location.href='./index.php?mod=loket&opt=proses_permohonan&opts=baru_sah&id=<?php echo $id_per; ?>';
		</script>
	<?php
	}

	$query=mysql_query("SELECT no_resi FROM tbl_berkas_sah ORDER BY no_resi DESC");
	...
	$qinsert="INSERT INTO tbl_berkas_sah
	(no_resi,tgl_masuk_berkas,id_perusahaan, nama_pemohon,alamat_pemohon,no_surat,tgl_surat,no_jamsostek,tenaga_lk,
	tenaga_pr,tenaga_lk_cabang,tenaga_pr_cabang,upah_bulanan_max,upah_bulanan_min,upah_harian_max,upah_harian_min,hub_tentu,
	hub_tak_tentu,konsep_pp,tgl_pp) 
	VALUES 
	('$no_resi',curdate(),'$id_per','$nama_pemohon','$alamat_pemohon','$no_surat', '$tgl_surat','$no_jamsostek',
	 '$tenaga_lk','$tenaga_pr','$tenaga_lk_cabang','$tenaga_pr_cabang','$upah_bulanan_max',
	 '$upah_bulanan_min','$upah_harian_max','$upah_harian_min','$hub_tentu','$hub_tak_tentu','$konsep_pp','$tgl_pp')";
	...
	if($d)
	{
		benar("./index.php?mod=loket&opt=proses_permohonan&opts=main_sah&no_resi=".$no_resi);
		exit();
	}
	else
	{
...
else if((int)$_GET["mode"] == 2)
{
	...
	?>
		<script type="text/javascript">
			alert('Isi dengan lengkap !');
			document.location.href='./index.php?mod=loket&opt=proses_permohonan&opts=edit_sah&no_resi=<?php echo $no_resi; ?>';
		</script>
	<?php
	}	
	$qryupdate="update tbl_berkas_sah set nama_pemohon='$nama_pemohon',alamat_pemohon='$alamat_pemohon',no_surat='$no_surat',tgl_surat='$tgl_surat',no_jamsostek='$no_jamsostek',
				...
				konsep_pp='$konsep_pp',tgl_pp='$tgl_pp' where no_resi='$no_resi'";
	...
	{
		salah("./index.php?mod=loket&opt=proses_permohonan&opts=edit_sah&no_resi=".$no_resi);
		exit();
	}
}
else
