...
<script type="text/javascript">
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/pengawas/hasil_lap/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=pengawas&opt=hasil_lap';
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '132' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '132' and isDisposisi='0'");
				...
				<?php 
					...
					echo "<tr>
						...
						while($set = mysql_fetch_array($query))
						{
							...
							echo "
								<td width='35px'> <input type='button' value='LAPORAN' onClick=$onc1></td>
								<td width='35px'> <input type='button' value='PROSES' onClick=$onc></td>
								</tr>";
							$i++;
					echo "
					<tr id='div_if'>
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/pengawas/hasil_lap/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/pengawas/hasil_lap/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/pengawas/hasil_lap/terima2.php');
		else
