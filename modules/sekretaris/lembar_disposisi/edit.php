...
if($jenis_urus=='04' or $jenis_urus=='05')
{
	if($sblm==77)
	{
		if($sta=='tolak')
		{
			$qry = "select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses_lanjutan='21'";
		}
		else
		{
			$qry = "select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses_lanjutan='410'";
		}
...
elseif($jenis_urus=='06')
{
		...
		{
			$qry = "select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses_lanjutan='38'";
		}
}
else
{
...
<fieldset style='width:90%; margin-left:40px; padding-left:20px;min-height:40px; margin-bottom:10px;padding-right:20px;'>
    ...
    <br />
    <br />     
<form action="./modules/sekretaris/lembar_disposisi/disposisi_validate.php?link=<?php echo $resi; ?>&id_user=<?php echo $userid; ?>" method="POST">
<table>
<tr>
	<td>DISPOSISI
