...
<script type="text/javascript">
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kasi/cek_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1300px" frameborder="0" scrolling="no" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kasi&opt=cek_berkas';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat;'> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>PROSES MEDIASI DAN MEMILIH MEDIATOR</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <a href="./index.php?mod=kasi&opt=main" id="linkutama"> BERANDA LOGIN KASI PERSELISIHAN</a> <span> 
                  <img src="./image/panah.gif"  /> PROSES MEDIASI DAN MEMILIH MEDIATOR 
...
<tr>
	<td>
     <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>PROSES MEDIASI DAN MEMILIH MEDIATOR</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '51' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg = '51' and isDisposisi='0'");
			...
			?>
			<table border='1' class="tblisi" cellspacing="0" style='width:100%; '>
					<tr>
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kasi/cek_berkas/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kasi/cek_berkas/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kasi/cek_berkas/terima2.php');
		else
