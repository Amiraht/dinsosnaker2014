...
<script type="text/javascript">
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kabidhubsaker/penugasan/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1300px" frameborder="0" scrolling="no" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kabidhubsaker&opt=penugasan';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>PROSES UNTUK MEDIASI</b></a></td>
            </tr>
            <tr>
...
<tr>
    ...
    <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>PROSES UNTUK MEDIASI</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('412') and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg in('412') and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg in('412') and isDisposisi='0' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg in('412') and isDisposisi='0'
											LIMIT ".$posisi.",".$batas."");
						}
				...
				<?php 
					...
					echo "<tr >
						...
						while($set = mysql_fetch_array($query))
						{
							...
							echo "<td> $set[6]</td>"; 
							echo "
								<td align='center'> <input type='button' value='PROSES' onClick=$onc></td>
								</tr>";
							$i++;
					echo "
					<tr id='div_if'>
...
<?php
		...
		if($_GET["opts"] == 'tolak')
			include('./modules/kabidhubsaker/penugasan/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kabidhubsaker/penugasan/terima.php');
		else
