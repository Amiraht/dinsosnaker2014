...
<script type="text/javascript">
	...
	function ket(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/stafkasihubsaker/cek_berkas/sk_wl/index.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/stafkasihubsaker/cek_berkas/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=stafkasihubsaker&opt=cek_berkas';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#">PEMBUATAN SK PP DAN PKB</a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <img src="./image/panah.gif"  /><a href="./index.php?mod=stafkasihubsaker&opt=main" id="linkutama"> BERANDA LOGIN STAF KASI HUBINSAKER</a> <img src="./image/panah.gif"  /><a href="" id="linkutama"> PEMBUATAN SK PP DAN PKB</a> 
                 </td>
...
<tr><td style="float:right;">
					<img width="90" height="29" 
						onclick="document.location.href='./index.php?mod=stafkasihubsaker&opt=main'" 
...
<tr>
	<td>
     <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>PEMBUATAN SK PP DAN PKB</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '161' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '161' and isDisposisi='0'");
				...
				<?php 
					...
					echo "<tr>
						...
						while($set = mysql_fetch_array($query))
						{
							...
							$onc1 = "ket('".$set['no_resi']."','".$_SESSION["id_user"]."')";
							$no=0;
							if($halaman>1)
							{
								$no=($halaman-1)*20;
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/stafkasihubsaker/cek_berkas/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/stafkasihubsaker/cek_berkas/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/stafkasihubsaker/cek_berkas/terima2.php');
		else
