...
<?php
    ...
    $whr = "";
     ...
     $sql = "SELECT
            	hasil.skpd, hasil.kode_skpd, SUM(hasil.sudah_isi) AS sudah_isi, SUM(hasil.belum_isi) AS belum_isi, SUM(hasil.belum_supervisi) AS belum_supervisi,
            ...
            FROM
            	(
            		SELECT
            			...
            			CASE
            				WHEN d.id_skp IS NULL THEN 0
            				ELSE 1
            			END AS sudah_isi,
                        CASE
            				WHEN d.id_skp IS NULL THEN 1
            				ELSE 0
            			END AS belum_isi,
            			CASE
            				WHEN d.status_supervisi <> 3 THEN 1
            				ELSE 0
            		...
            		FROM
            			...
            			LEFT JOIN tbl_skp_dp3_upload_arange d ON (b.id_skp = d.id_skp " . $pilih . " )
...
<?php
    ...
    if($_POST["kode_skpd"] != "all"){
        ...
        while($ds = mysql_fetch_array($res)){
            ...
            if(($no - 1) % 20 == 0){
                $html .= "<h3 style='text-align: center;'>REKAPITULASI TELAH MENGUPLOAD SKP " . strtoupper($nama_skpd . " TAHUN " . $_POST["tahun"]) . "</h3>";
                $html .= "<table width='100%' border='1px' cellspacing='0' cellpadding='5' style='border-collapse: collapse; font-family: sans-serif; font-size: 8pt; page-break-after: always'>";
                    $html .= "<tr>";
                        ...
                        $html .= "<th width='7%'>BELUM MENGUPLOAD</th>";
                        $html .= "<th width='7%'>TELAH MENGUPLOAD</th>";
                        $html .= "<th width='7%'>TELAH MENGUPLOAD<BR/>BELUM DI SUPERVISI</th>";
                        $html .= "<th width='7%'>TELAH MENGUPLOAD<BR/>TELAH DI SUPERVISI</th>";
                    $html .= "</tr>";
                    $html .= "<tr>";
