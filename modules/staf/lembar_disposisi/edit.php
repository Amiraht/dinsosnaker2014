...
if($bz[0]=='63')
{
		$nmpri=mysql_query("select hasil_mediasi from tbl_berkas_pengaduan where no_resi='$resi'");
		$rnmpri = mysql_fetch_array($nmpri);
		$hasil=$rnmpri["hasil_mediasi"];
		if($hasil=='3')
		{
			$cz = mysql_query("select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses = '".$bz[0]."' and id_pengguna='3'");
		}
		else
		{
			$cz = mysql_query("select id_pengguna,id_proses_lanjutan from tbl_alur_baru where id_proses = '".$bz[0]."' and id_pengguna<>'3'");
...
<fieldset style='width:90%; margin-left:40px; padding-left:20px;min-height:40px; margin-bottom:10px;padding-right:20px;'>
    ...
    <br />
    <br />     
<form action="./modules/staf/lembar_disposisi/disposisi_validate.php?link=<?php echo $resi; ?>&id_user=<?php echo $userid; ?>" method="POST">
<table>
<tr>
	<td>DISPOSISI
