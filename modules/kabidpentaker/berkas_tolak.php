...
<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kabidpentaker&opt=berkas_tolak';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kabidpentaker/berkas_tolak/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>BERKAS DITOLAK</b></a></td>
            </tr>
            <tr>
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '48' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from tbl_info_berkas where id_proses_skrg = '48' and isDisposisi='0'");
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kabidpentaker/berkas_tolak/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kabidpentaker/berkas_tolak/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kabidpentaker/berkas_tolak/terima2.php');
		else
