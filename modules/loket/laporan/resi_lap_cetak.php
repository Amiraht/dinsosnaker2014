...
if ($_GET["mode"]=='1')
{
	...
	HeaderingExcel('Laporan_resi.xls');
	...
	// tabel title=================================================================================
	$worksheet1->write_string(2,1,"Laporan_resi",$fBesar);
	...
	{						
		if(!empty($_GET["cari"]))
			$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where  isValid='1' and pemohon like '%".$_GET["cari"]."%'");
		elseif(!empty($_GET["no_resi"]))
		{
			$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where  isValid='1' and no_resi like '%".$_GET["no_resi"]."%'");
		}
		else
		{
			$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where isValid='1'");
		}
	}	
