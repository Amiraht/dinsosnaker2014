...
if ($_GET["mode"]=='1')
{
	...
	HeaderingExcel('Laporan_wl_belum.xls');
	...
	// tabel title=================================================================================
	$worksheet1->write_string(2,1,"LAPORAN SK WAJIB LAPOR YANG BELUM MELAPOR",$fBesar);
	...
	$worksheet1->write_string(4,4,"Nama Perusahaan",$fList_title);
	$worksheet1->write_string(4,5,"Alamat",$fList_title);
	$worksheet1->write_string(4,6,"No SK Wajib Lapor",$fList_title);
	$worksheet1->write_string(4,7,"Tanggal SK Wajib Lapor",$fList_title);
	$worksheet1->write_string(4,8,"Tanggal Berakhir",$fList_title);
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$query = mysql_query("select no_resi,tgl_masuk,nama,alamat,no_sk,berlaku,sampai from vw_sk_wl where sampai <'$tgl_sekarang' and ".$up."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk,nama,alamat,no_sk,berlaku,sampai from vw_sk_wl where sampai <'$tgl_sekarang'");
	}	
