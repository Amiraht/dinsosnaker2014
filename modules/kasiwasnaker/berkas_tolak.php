...
<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=kasiwasnaker&opt=berkas_tolak';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/kasiwasnaker/berkas_tolak/tampil.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1400px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
...
<td style="float:right;">
					<img width="90" height="29" 
						onclick="document.location.href='./index.php?mod=kasiwasnaker&opt=main'" 
...
<tr>
	...
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '122' and isDisposisi='0'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '122' and isDisposisi='0'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '122' and isDisposisi='0' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg = '122' and isDisposisi='0'
											LIMIT ".$posisi.",".$batas."");
						}
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/kasiwasnaker/berkas_tolak/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/kasiwasnaker/berkas_tolak/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/kasiwasnaker/berkas_tolak/terima2.php');
		else
