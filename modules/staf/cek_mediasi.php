...
<script type="text/javascript">
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=staf&opt=cek_mediasi';
	...
	function tampil(c,uid)
	{
			...
			if(t.innerHTML != ''){
				...
				t.innerHTML='<iframe src="./modules/staf/cek_mediasi/edit.php?link='+c+'&id_user='+uid+'" width="100%" style="height:1000px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#">HASIL MEDIASI</a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
                  ...
                  <img src="./image/panah.gif"  /><a href="" id="linkutama"> HASIL MEDIASI</a> 
                 </td>
...
<tr>
	<td>
		<fieldset>
		<legend>HASIL MEDIASI</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '63' and isDisposisi='0' and mediator='".$userid."'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_proses_skrg = '63' and isDisposisi='0' and mediator='$userid'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_proses_skrg = '63' and isDisposisi='0' and mediator='$userid' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
							$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas WHERE		
												id_proses_skrg = '63' and isDisposisi='0' and mediator='$userid'
											LIMIT ".$posisi.",".$batas."");
						}
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/staf/cek_mediasi/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/staf/cek_mediasi/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/staf/cek_mediasi/terima2.php');
		else if($_GET["opts"] == 'laporan')
			include('./modules/staf/cek_mediasi/edit_pengaduan.php');
		else
