...
<script type="text/javascript">	
		function cari_per()
	{
		...
		var s;
		s='index.php?mod=dinsos&do=data_perusahaan';	
		//if(y != '')
			s=s+'&cari='+a+'&usaha='+b+'&modal='+c+'&jenis='+d+'&kel='+e+'&kec='+f+'&tgl='+g+'&klui='+h+'&stat_milik='+i;
	...
	function lihat_detail(c,d)
	{
		...
		if(t.innerHTML != '')
			...
			t.innerHTML='<iframe src="./modules/dinsos/data_perusahaan/detail2.php?id='+c+'&sumber='+d+'&hidden=b" width="100%" style="height:900px" frameborder="0" scrolling="yes" id="iframe_detail"></iframe>';
	}
	...
	function edit(c,d)
	{
		...
		if(t.innerHTML != '')
			...
			t.innerHTML='<iframe src="./modules/dinsos/data_perusahaan/edit.php?id='+c+'&sumber='+d+'&hidden=b" width="100%" style="height:1000px" frameborder="0" scrolling="no" id="iframe_detail"></iframe>';
	}
...
<?php
	$filter = "nama LIKE '%" . $_GET["cari"] . "%' ";
						...
						if(isset($_GET["date"]) && $_GET["date"] != "0" && $_GET["date"] != "")
							$filter .= "AND tgl_diri = '" . $_GET["date"] . "'";
						if(isset($_GET["klui"]) && $_GET["klui"] != "" && $_GET["klui"] != "0")
							$filter .= "AND klui = '" . $_GET["klui"] . "'";
					
							
	$data2 = "select * from db_dinsos where $filter";
...
<table width="1024" border="0" cellspacing="2" cellpadding="5">
  <tr >
    ...
    <tr>
    	<td style="float:right; padding-right:11px;">
                        ...
                        <table border="0" cellpadding="3">
                        	<tr>
                              <td width="250">NAMA PERUSAHAAN</td>
                           	  <td width="144" ><input type='text' name='cari' placeholder='Cari Nama Perusahaan' /></td>
                              <td width="240">TANGGAL PENDAFTARAN</td>
                           	  <td width="144" ><input size="20" type='date' name='tgl'> 
                           	    <span class="style1">format tanggal : yyyy-mm-dd</span></td>
                          </tr>
                        	<tr>
                        	  <td>KLUI</td>
                        	  <td><input type='text' name='klui' placeholder='Cari Klui' /></td>
                        	  <td>STATUS USAHA</td>
                            ...
                            <tr id='detail'>
                            <td colspan="4" align="center"><span style="padding-top:5px;">
                              <br/><input type='button' value='CARI PERUSAHAAN' onclick="cari_per();" />
                            </span></td>
                            </tr>
                        </table>
                     <br />
 ...
 <?php
		...
		if(!empty($_GET["cari"]))
			{
				$jlh_data = mysql_query("select count(id_perusahaan) as jumlah from db_dinsos where $filter");
			}
		else
				$jlh_data = mysql_query("select count(id_perusahaan) as jumlah from db_dinsos");
			...
			if(!empty($_GET["cari"]))
			{
				$qsrt = mysql_query("select * from db_dinsos where $filter order by nama asc limit ".$posisi.",".$batas."");
			}
			else
				$qsrt = mysql_query("select * from db_dinsos order by nama asc limit ".$posisi.",".$batas."");
			...
			$data = "select 
								a.id_perusahaan, a.nama, b.jenis, a.klui, a.alamat 
								from db_dinsos a
...
<?php 
	...
	echo "<tr>
		...
		<tr>
			...
			<td> $set[3] </td>
			<td> $set[4]</td>
			<td align='center' width='35px'> <a href='javascript:lihat_detail($set[0],\"DINSOSNAKER\");'>LIHAT</a> </td>
			<td align='center' width='35px'> <a href='javascript:edit($set[0],\"DINSOSNAKER\");'>EDIT</a> </td>";
	?>
			<td width="45px" align="center" valign="middle"> 
            	<a href="./modules/dinsos/data_perusahaan/hapus.php?optss=hapus&id=<?php echo $set[0]; ?>" onClick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
			HAPUS</a></td>
	<?php echo"
		</tr>
		<tr>
