...
<script type="text/javascript">
	function cetak(u)
	{
		s='./index.php?mod=stafkasilatih&opt=cetak&link='+u;
		window.open(s)
	...
	function Kirim_Cari()
	{
		...
		var s;
		s='./index.php?mod=stafkasilatih&opt=cetak_ulang';
...
<tr >
    <td colspan="2"><div id='header' style='background:url(./image/coba/header2.png) no-repeat; '> 
      <table border="0" id='atasan'>
        	<tr>
		         <td colspan='2' style="text-align:right; padding-right:10px; padding-bottom:50px;"><a style="color:#AA9F00;" href="#"><b>PRINT ULANG SK IPLK</b></a></td>
            </tr>
            <tr>
            	 <td style="text-align:left; padding-left:10px; border-left:0px solid black;">
				  <img src="./image/panah.gif"  /><a href="./index.php?mod=stafkasilatih&opt=main" id="linkutama"> BERANDA LOGIN STAF KASI INSTRUKTUR</a>
                  <img src="./image/panah.gif"  /><a href="" id="linkutama"> PRINT ULANG SK IPLK</a> 
                 </td>
...
<tr>
    ...
    <div id='content' style='margin-left:2%;margin-bottom:20px;width:98%;'>
		...
		<fieldset>
		<legend>PRINT ULANG SK IPLK</legend>
	<?php
			if(!empty($_GET["cari"]))
				{
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_pengurusan='06'");
				}
				else
					$d = mysql_query("select count(*) as jumlah from vw_info_berkas where id_pengurusan='06'");
				...
				if(isset($_GET["cari"]))
					{
						$query = mysql_query("select no_resi,tgl_masuk,pengurusan,nama,pemohon,alamat,alamat_pemohon from vw_info_berkas where nama like '%".$_GET["cari"]."%' AND id_pengurusan='06' LIMIT ".$posisi.",".$batas."");
						}
					else
						{
...
<?php
	...
	{
		if($_GET["opts"] == 'tolak')
			include('./modules/stafkasilatih/cetak_ulang/tolak.php');
		else if($_GET["opts"] == 'terima')
			include('./modules/stafkasilatih/cetak_ulang/terima.php');
		else if($_GET["opts"] == 'terima2')
			include('./modules/stafkasilatih/cetak_ulang/terima2.php');
		else
