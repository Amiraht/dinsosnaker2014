...
if ($_GET["mode"]=='1')
{
	...
	// tabel title=================================================================================
	$worksheet1->write_string(2,1,"LAPORAN SK WAJIB LAPOR",$fBesar);
	...
	if(!empty($_GET["tgl1"]) || !empty($_GET["tgl2"]) || !empty($_GET["cari"]) || !empty($_GET["no_resi"]))
	{
		$query = mysql_query("select no_resi,tgl_masuk,nama,alamat,no_sk,berlaku,sampai from vw_sk_wl where nama is not null and ".$up."");
	}
	else
	{						
		$query = mysql_query("select no_resi,tgl_masuk,nama,alamat,no_sk,berlaku,sampai from vw_sk_wl where nama is not null");
	}	
